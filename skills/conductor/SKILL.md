...
| `xhigh`  | hard judgment, taste-critical surfaces, the Astra cross-family review |
...
- **Reviews of plans/implementations** (changed 2026-09-05): `fable` at `high`/`max` is the Claude judgment lane — the bundled `verify-lane` agent (Fable 5.1 at `max`). A review is read-heavy (one cached diff re-read many times, little output), which is where Fable's economics beat Opus (cache reads $0.25 vs $0.50, far fewer tokens per task), and Fable leads the two axes a review is about — reasoning and steerability — at the same cost per task. `opus` at `max` is the *second* Claude opinion on a close call. `gpt-6-astra` at `xhigh` via `codex-review` is the **co-equal** cross-family review — run it beside Fable on anything that ships. Never review at `low`.
...
| Judgment review | Fable 5.1 (`max`) — `verify-lane`; Opus 5 at `max` only as a second opinion on a close call | `Agent` with the bundled `verify-lane` agent | Adversarial review of a diff, a plan or a finding against its acceptance criteria — read-heavy, so Fable's economics win; verdict + ranked findings with file:line |
| Diff review (cross-family) | GPT-6 Astra (`xhigh`) | `codex-review` skill (or `Bash` → `ask-codex --readonly --effort xhigh`) | Co-equal cross-family review of a diff — run beside the Fable judgment lane on anything that ships |
| Runtime verification (mechanics) | GPT-6 Astra (`medium`) | `codex-computer-use` skill | **The default runtime lane.** Unattended confirmation that the *running* thing works — launch it, drive `agent-browser`/Playwright/simulator, or drive a native GUI through `node_repl` + `@oai/sky`, capture screenshots and video into a directory you then Read, assert against the acceptance check |
...
**Bundled agents.** This plugin ships four pre-tuned lane agents with `model:` and `effort:` already set — `design-lane` (opus/xhigh), `bulk-lane` (opus/low), `verify-lane` (fable/max — the Claude judgment review; changed 2026-09-05), `write-lane` (fable/high). Use them via the `Agent` tool when you want the effort pinned regardless of session effort; the plain `Agent` tool has no `effort` parameter of its own, so a subagent dispatched without one of these inherits the session's effort.
...
- **Styling:** **light mode, always** (Ivan, 5 Sep 2026 — off-white ground, dark ink, sage accent; never a dark palette), refined, data-dense — match the executive register (headings distinct from body, monospace for paths/metrics). One self-contained file; it must render offline except for an optional mermaid CDN tag.
